...
class clip_only_validations(nn.Module):
    """
    Simplified validation for clip-only model.
    Only uses clip-level features for evaluation.
    """
    def __init__(self, cfg):
        super(clip_only_validations, self).__init__()
        self.cfg = cfg

    def forward(self, model, context_dataloader, query_eval_loader):
        """
        Main validation function for clip-only model.
        """
        model.eval()
    ...
    def compute_query2ctx_info(self, model, query_eval_loader, ctx_info):
        """
        Compute query-to-context similarity scores using only clip-level features.
        """
        query_metas = []
        score_sum = []
        ...
        for idx, batch in tqdm(enumerate(query_eval_loader), desc="Computing q embedding", total=len(query_eval_loader)):
            ...
            # Only get clip-level scores (no frame scores)
            _clip_scale_scores = model.get_pred_from_raw_query(
                query_feat, query_mask, None, ctx_info["video_proposal_feat"])
            
            # Use only clip-level scores (no frame-level weighting)
            score_sum.append(_clip_scale_scores)

        score_sum = torch.cat(score_sum, dim=0)
        
        # Debug: Check score distribution
        print(f"Clip-Only similarity scores - Min: {score_sum.min().item():.4f}, "
              f"Max: {score_sum.max().item():.4f}, "
              f"Mean: {score_sum.mean().item():.4f}, "
              f"Std: {score_sum.std().item():.4f}")
        
        return score_sum, query_metas

    def compute_context_info(self, model, context_dataloader):
        """
        Compute context information using only clip-level features.
        """
        ...
        for idx, batch in tqdm(enumerate(context_dataloader), desc="Computing query2video scores",
            ...
            # Note: frame_video_feat and frame_mask are not used in clip-only model
            
            # Only encode clip-level context
            _video_proposal_feat = model.encode_clip_context(clip_video_feat_)
